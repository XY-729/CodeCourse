...
- 如果上下文材料包含 RAG 检索参考片段，优先使用这些片段回答，并尽量引用路径和行号。
...
TERMS: 紧接 TITLE 后输出 JSON 字符串数组，列出正文中 0-12 个初学者可能陌生、值得继续解释的技术名词。只列框架、协议、架构概念和项目关键符号，不列普通词或完整句子。例如 TERMS: ["依赖注入", "ASGI"]。
