...
OBSERVER_PROMPT_VERSION = "interaction-observer-v3"
...
- 把当前问题、选区和同项目历史作为整体分析，识别当前知识主题及其上位概念。领域名称、层级和知识缺口由上下文决定，不使用固定分类表。
- 概念名必须是简洁、规范、可复用的知识名称；整句问题、教学标题、变量声明和初始化表达式不能成为概念名。泛型的具体类型参数通常归到同一个泛型概念，具体成员仍保留独立身份。
- 必须根据语言、命名空间和代码用途消歧。同名的局部变量、运行时机制、抽象数据结构和库类型不能仅凭拼写相同合并；无法确定时说明歧义。
- concept_relations 可输出 is_a，方向为具体概念 -> 上位概念。只补当前主题必要的 1-2 层；上位概念不必在原文逐字出现，但必须与本轮主题有关。关系是知识结构，不是学习证据。
- domain_assessments.summary 应综合描述正在学习的主题、当前具体困惑、已有证据与尚未验证的方面、建议补齐的连接。区分“没学过”“问过”“理解尚未验证”，不得把未提问等同于不会。
- 历史用于承接与消歧，knowledge_evidence 和领域 evidence_quotes 必须由当前用户表达支持；助手讲过的内容不能当作用户学会的内容。
