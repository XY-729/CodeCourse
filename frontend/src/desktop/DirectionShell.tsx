...
export default function DirectionShell(props: Props) {
  ...
  return <div className={`direction-shell ${shown === 'reader' ? 'is-reader' : ''}`} ref={root}>
    ...
    {shown === 'generate' && <div className="direction-generation-modes">{([
      ['outline','学习总纲','规划一条完整的学习路线'],
      ['lesson','当前课件',props.canGenerateLesson ? '围绕当前章节展开讲解' : '先在阅读区打开一份课件'],
      ['brief','粗略介绍',props.canGenerateFile ? '快速了解文件的职责与结构' : '先在阅读区打开一个源码文件'],
      ['detailed','详细分析',props.canGenerateFile ? '深入理解实现与上下文' : '先在阅读区打开一个源码文件'],
    ] as const).map(([intent,label,description],i) => <button key={intent} className={props.generationIntent === intent ? 'selected' : ''} aria-pressed={props.generationIntent === intent} disabled={!props.project || props.loading || (intent === 'lesson' && !props.canGenerateLesson) || ((intent === 'brief' || intent === 'detailed') && !props.canGenerateFile)} onClick={() => props.onOpenGeneration(intent)}><span>0{i + 1}</span><div className="direction-generation-mode-copy"><strong>{label}</strong><small>{description}</small></div><ArrowUpRight size={18}/></button>)}</div>}
