...
describe("TeachingContinuityCards", () => {
  ...
  it("merges the exact AI topic across sessions without keyword renaming", () => {
    ...
    const threads: QAThreadSummary[] = [shared, unique].map((item) => ({
      sessionId: item.session_id!, topic: "智能指针", progressSummary: "", unresolvedPoints: [], turnCount: 1,
    ...
    expect(groups[0].summary.topic).toBe("智能指针");
